...
class HideActionSubscriber implements EventSubscriberInterface
{
    public function onBeforeCrudActionEvent(BeforeCrudActionEvent $event): void
    {
        ...
        if ($crudDto->getEntityFqcn() !== Order::class) {
            return;
        }

        // disable action entirely delete, detail, edit
        $entity = $adminContext->getEntity()->getInstance();
        if ($entity instanceof Order && $entity->getStatus()) {
            $crudDto->getActionsConfig()->disableActions([Action::EDIT, Action::DELETE]);
        }
        ...
        $deleteAction->setDisplayCallable(function(Order $entity) {
